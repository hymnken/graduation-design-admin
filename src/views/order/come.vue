<template>
  ...
  <a-table :data-source="tableData" :pagination="false">
    ...
    <a-table-column key="earnest_money" title="定金" data-index="earnest_money">
      <template #default="{ record }">{{ record.earnest_money }}</template>
    </a-table-column>
    <a-table-column key="earnest_money" title="尾款" data-index="earnest_money">
      <template #default="{ record }">{{ record.surplus_money }}</template>
    </a-table-column>
    <a-table-column key="earnest_money" title="总金额" data-index="earnest_money">
      <template #default="{ record }">{{ record.total_money }}</template>
    </a-table-column>
    ...
    <a-table-column key="action" title="操作">
      <template #default="{ record }">
        <a-space>
          <a-button v-if="record.status == 3" type="link" v-permission="['sys_order']" @click="handleComeIn(record)"
            >办理入住</a-button
          >
          <!-- <a-button v-if="record.status == 4" type="link" v-permission="['sys_order']" @click="handleRenew(record)"
            >续住</a-button
          > -->
          <a-button v-if="record.status == 4" type="link" v-permission="['sys_order']" @click="handleComplete(record)"
            >完成订单</a-button
          >
          <a-popconfirm title="确认要删除该订单?" ok-text="确认" cancel-text="再想想" @confirm="handleDelete(record)">
...
const queryList = () => {
  console.log(`ddd`)
  list({ page: pageIndex.value, isCome: true }).then((res) => {
