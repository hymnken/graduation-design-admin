  ...
  <div>
    ...
    <a-table :data-source="tableData" :pagination="false">
      ...
      <a-table-column key="action" title="操作">
        <template #default="{ record }">
          <a-button type="link" v-permission="['sys_house']" @click="handleHouseOff(record)">{{
    ...
    <a-modal @ok="handleSave" v-model:visible="showDetailModal" width="800px" :title="houseData.id ? '编辑民宿' : '新增民宿'">
      <a-form :model="houseData" name="basic" autocomplete="off" @finish="onFinish" @finishFailed="onFinishFailed">
        ...
        <a-form-item label="详细图片" name="cover">
          <div class="flex">
            <div v-for="item in houseImages" :key="item" class="img-item" @click="removeImageItem(item)">
              <img :src="item" />
            </div>
            <a-upload
              class="image-uploader"
...
const handleHouseOff = (row) => {
  console.log('%c [ row ]-165', 'font-size:13px; background:pink; color:#bf2c9f;', row)
  houseOff({
    id: row.id,
